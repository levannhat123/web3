...
async function updateUser(userId) {
  ...
  try {
      console.log("Sending transaction to update user...");
      const tx = await contract.updateUser(userId, userName, userHealth, userStrength);
      await tx.wait(); // Chờ giao dịch được xác nhận
      alert("User updated successfully!");
      window.location.href = "user_management.html"; // Chuyển hướng sau khi cập nhật
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  const userId = new URLSearchParams(window.location.search).get('userId'); // Lấy userId từ URL

  if (userId) {
      await connectToBlockchain(); // Kết nối blockchain
      await loadUserData(userId); // Tải thông tin người dùng
      document.getElementById("save").addEventListener("click", () => updateUser(userId));
  } else {
